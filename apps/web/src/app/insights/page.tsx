...
const MARKET_METRICS = [
  {label:'Global FDI Flow',    v:'$2.8T',  chg:'+11%', c:'#00ffc8', sub:'2025 full year'},
  {label:'ASEAN Share',        v:'8.2%',   chg:'+1.4pp',c:'#00d4ff', sub:'Of global FDI'},
  ...
  {label:'Top Source: USA',    v:'$412B',  chg:'+8%',  c:'#00ffc8', sub:'Outbound FDI'},
];

const TYPE_STYLE: Record<string,{bg:string,c:string}> = {
  MACRO:  {bg:'rgba(0,180,216,0.1)', c:'#00d4ff'},
    ...
    <div style={{minHeight:'100vh',background:'#020c14',fontFamily:"'Inter','Helvetica Neue',sans-serif"}}>
      ...
      <div style={{background:'linear-gradient(135deg,#020c14,#060f1a)',padding:'24px',borderBottom:'1px solid rgba(0,255,200,0.06)',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{maxWidth:'1540px',margin:'0 auto',position:'relative'}}>
          <div style={{fontSize:'10px',fontWeight:800,color:'rgba(0,255,200,0.5)',letterSpacing:'0.2em',marginBottom:'6px',fontFamily:"'Orbitron','Inter',sans-serif"}}>MARKET INSIGHTS</div>
          <h1 style={{fontSize:'26px',fontWeight:900,color:'#e8f4f8',marginBottom:'4px'}}>Global FDI Market Intelligence</h1>
          <p style={{fontSize:'13px',color:'rgba(232,244,248,0.45)'}}>Macro trends · Policy shifts · Sector analysis · Risk monitoring</p>
        </div>
      </div>
      ...
      <div style={{maxWidth:'1540px',margin:'0 auto',padding:'20px 24px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(6,1fr)',gap:'10px',marginBottom:'18px'}}>
          {MARKET_METRICS.map(({label,v,chg,c,sub}) => (
            <div key={label} style={{padding:'14px',background:'rgba(10,22,40,0.8)',border:'1px solid rgba(0,180,216,0.08)',borderRadius:'10px',borderTop:'2px solid '+c}}>
              <div style={{fontSize:'9px',color:'rgba(232,244,248,0.3)',marginBottom:'5px',textTransform:'uppercase',letterSpacing:'0.06em'}}>{label}</div>
              <div style={{fontSize:'20px',fontWeight:900,color:c,fontFamily:"'JetBrains Mono',monospace"}}>{v}</div>
              <div style={{display:'flex',gap:'5px',alignItems:'center',marginTop:'2px'}}>
                <span style={{fontSize:'10px',fontWeight:700,color:'#00ffc8'}}>{chg}</span>
                <span style={{fontSize:'9px',color:'rgba(232,244,248,0.25)'}}>{sub}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{display:'flex',gap:'8px',marginBottom:'16px',flexWrap:'wrap'}}>
          {['ALL','MACRO','POLICY','SECTOR','RISK','Asia Pacific','Middle East','Europe','Global'].map(f => (
            <button key={f} onClick={()=>setFilter(f)}
              style={{padding:'6px 14px',borderRadius:'20px',border:`1px solid ${filter===f?'rgba(0,255,200,0.3)':'rgba(255,255,255,0.07)'}`,background:filter===f?'rgba(0,255,200,0.08)':'rgba(255,255,255,0.03)',cursor:'pointer',fontSize:'11px',fontWeight:filter===f?700:400,color:filter===f?'#00ffc8':'rgba(232,244,248,0.45)',fontFamily:"'Inter',sans-serif",transition:'all 150ms'}}>
              {f}
            </button>
          ))}
        </div>
              ...
              <div key={insight.id}
                ...
                <div style={{display:'flex',gap:'10px',alignItems:'flex-start',marginBottom:'10px'}}>
                  ...
                  <div style={{flex:1}}>
                    <div style={{display:'flex',gap:'6px',alignItems:'center',marginBottom:'5px',flexWrap:'wrap'}}>
                      ...
                      <span style={{fontSize:'9px',color:'rgba(232,244,248,0.3)'}}>{insight.region}</span>
                      {insight.trending && <span style={{fontSize:'9px',fontWeight:800,color:'#00ffc8',padding:'1px 6px',background:'rgba(0,255,200,0.08)',borderRadius:'8px',display:'flex',alignItems:'center',gap:'3px'}}><TrendingUp size={8}/>TRENDING</span>}
                    </div>
                    <div style={{fontSize:'14px',fontWeight:700,color:'rgba(232,244,248,0.9)',lineHeight:1.35,marginBottom:'6px'}}>{insight.title}</div>
                    <div style={{fontSize:'12px',color:'rgba(232,244,248,0.5)',lineHeight:1.65}}>{isSel ? insight.summary : insight.summary.slice(0,110)+'...'}</div>
                  </div>
                </div>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  <span style={{fontSize:'10px',color:'rgba(232,244,248,0.25)',fontFamily:"'JetBrains Mono',monospace"}}>{insight.date}</span>
                ...
                {isSel && (
                  <div style={{marginTop:'12px',paddingTop:'12px',borderTop:'1px solid rgba(0,255,200,0.08)',display:'flex',gap:'8px'}}>
                    <Link href="/signals" onClick={e=>e.stopPropagation()} style={{padding:'6px 14px',background:'rgba(0,255,200,0.07)',border:'1px solid rgba(0,255,200,0.18)',borderRadius:'7px',textDecoration:'none',fontSize:'11px',fontWeight:700,color:'#00ffc8'}}>Related Signals</Link>
                    <Link href="/investment-analysis" onClick={e=>e.stopPropagation()} style={{padding:'6px 14px',background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:'7px',textDecoration:'none',fontSize:'11px',fontWeight:600,color:'rgba(232,244,248,0.6)'}}>Economy Analysis</Link>
                  </div>
                )}
              </div>
            );
