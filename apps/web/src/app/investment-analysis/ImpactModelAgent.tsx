...
Respond ONLY with a JSON object, no markdown, no backticks:
{
  ...
  const metrics = result ? [
    ['GDP Contribution',result.gdp,'10-year horizon','#00ffc8'],
    ['Direct Jobs',result.jobs,'+ 2× indirect','#00d4ff'],
    ...
    ['Projected IRR',result.irr,'Post-incentive','#00ffc8'],
  ] : [];

  return (
    <div style={{display:'grid',gridTemplateColumns:'340px 1fr',gap:'16px',alignItems:'start'}}>
      {/* Config panel */}
      <div style={{background:'rgba(10,22,40,0.8)',border:'1px solid rgba(0,180,216,0.12)',borderRadius:'12px',padding:'22px'}}>
        ...
        <div style={{marginTop:'12px',fontSize:'10px',color:'rgba(232,244,248,0.25)',textAlign:'center',lineHeight:1.5}}>
      ...
      <div>
        {!result&&!loading&&(
          <div style={{background:'rgba(10,22,40,0.6)',border:'1px solid rgba(0,180,216,0.08)',borderRadius:'12px',padding:'48px',textAlign:'center'}}>
            ...
            <div style={{fontSize:'16px',fontWeight:700,color:'rgba(232,244,248,0.6)',marginBottom:'6px'}}>Configure your investment scenario</div>
        ...
        {loading&&(
          <div style={{background:'rgba(10,22,40,0.6)',border:'1px solid rgba(0,255,200,0.12)',borderRadius:'12px',padding:'48px',textAlign:'center'}}>
            ...
            <div style={{fontSize:'14px',fontWeight:700,color:'rgba(232,244,248,0.7)',marginBottom:'4px'}}>AI modelling {economy} {sector}...</div>
        ...
        {result&&(
          <div>
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'12px',marginBottom:'14px'}}>
              {metrics.map(([l,v,s,c]) => (
                <div key={String(l)} style={{padding:'16px',background:'rgba(10,22,40,0.8)',border:'1px solid '+c+'12',borderRadius:'10px',borderTop:'2px solid '+c}}>
                  ...
                  <div style={{fontSize:'10px',color:'rgba(232,244,248,0.3)'}}>{s as string}</div>
                </div>
              ))}
            </div>
            <div style={{background:'rgba(10,22,40,0.8)',border:'1px solid rgba(0,180,216,0.1)',borderRadius:'12px',padding:'18px',marginBottom:'12px'}}>
              ...
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse',fontSize:'12px'}}>
                  <thead>
                    <tr>
                      {['Metric',...(result.timeline[0]||[])].map(h=><th key={h} style={{padding:'8px 10px',fontWeight:700,color:'rgba(232,244,248,0.3)',textTransform:'uppercase',fontSize:'9px',letterSpacing:'0.07em',borderBottom:'1px solid rgba(0,255,200,0.06)',textAlign:h==='Metric'?'left':'center'}}>{h}</th>)}
                    </tr>
                  </thead>
                  <tbody>
                    {[['Revenue',...(result.timeline[1]||[])],['EBITDA',...(result.timeline[2]||[])],['Direct Jobs',...(result.timeline[3]||[])]].map(row=>(
                      <tr key={row[0]}>
                        <td style={{padding:'8px 10px',fontWeight:600,color:'rgba(232,244,248,0.6)',fontSize:'12px'}}>{row[0]}</td>
                        {row.slice(1).map((v,i)=><td key={i} style={{padding:'8px 10px',textAlign:'center',fontFamily:"'JetBrains Mono',monospace",fontSize:'12px',color:'rgba(232,244,248,0.7)'}}>{v}</td>)}
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div style={{padding:'16px',background:'rgba(0,255,200,0.04)',borderRadius:'10px',border:'1px solid rgba(0,255,200,0.12)',marginBottom:'12px'}}>
              <div style={{fontSize:'11px',fontWeight:800,color:'#00ffc8',marginBottom:'6px'}}>AI INVESTMENT VERDICT</div>
              <div style={{fontSize:'13px',color:'rgba(232,244,248,0.7)',lineHeight:1.75}}>{result.verdict}</div>
            </div>
            <div style={{display:'flex',gap:'10px'}}>
              ...
              <button onClick={runModel} style={{padding:'10px 18px',background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:'9px',cursor:'pointer',fontSize:'12px',fontWeight:600,color:'rgba(232,244,248,0.6)',fontFamily:"'Inter',sans-serif",display:'flex',alignItems:'center',gap:'6px'}}>
