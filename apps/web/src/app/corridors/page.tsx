...
const CORRIDORS = [
  {from:'China',to:'Vietnam',fromFlag:'🇨🇳',toFlag:'🇻🇳',flow:'$8.2B',momentum:'HOT',    color:'var(--accent-green)', sectors:['Electronics','EV Battery','Textiles'],  gosaFrom:64,gosaTo:79.4,driver:'China+1 relocation — Samsung, Intel suppliers relocating manufacturing',  growth:'+34%',signals:4},
  {from:'Japan',to:'Vietnam',fromFlag:'🇯🇵',toFlag:'🇻🇳',flow:'$5.1B',momentum:'RISING',  color:'var(--accent-blue)', sectors:['Auto Parts','Electronics','Retail'],      gosaFrom:81.4,gosaTo:79.4,driver:'Japan ASEAN diversification strategy, Toyota supply chain, convenience retail',growth:'+18%',signals:2},
  ...
  {from:'Europe',to:'UAE',   fromFlag:'🇪🇺',toFlag:'🇦🇪',flow:'$4.4B',momentum:'HOT',    color:'var(--accent-green)', sectors:['Finance','Logistics','AI Data Centers'],  gosaFrom:85,gosaTo:82.1,driver:'UAE as European HQ for MEA operations, DIFC financial cluster growth',        growth:'+22%',signals:3},
  ...
  {from:'Singapore',to:'Malaysia',fromFlag:'🇸🇬',toFlag:'🇲🇾',flow:'$5.8B',momentum:'HOT',color:'var(--accent-green)',sectors:['Data Centers','Semiconductors','Fintech'], gosaFrom:88.4,gosaTo:81.2,driver:'Singapore land scarcity driving data center spillover, JB-Singapore Special Economic Zone',growth:'+38%',signals:3},
  ...
  {from:'India',to:'UAE',    fromFlag:'🇮🇳',toFlag:'🇦🇪',flow:'$3.2B',momentum:'STABLE',  color:'var(--accent-blue)', sectors:['IT Services','Finance','Real Estate'],    gosaFrom:73.2,gosaTo:82.1,driver:'Indian diaspora investment, GIFT City-ADGM linkage, IT firm regional HQs',       growth:'+12%',signals:1},
  ...
  {from:'Japan',to:'Thailand',fromFlag:'🇯🇵',toFlag:'🇹🇭',flow:'$4.2B',momentum:'STABLE', color:'var(--accent-green)', sectors:['Automotive','Electronics','Retail'],      gosaFrom:81.4,gosaTo:80.7,driver:'50yr Japan-Thailand investment relationship, auto cluster Toyota and Honda',   growth:'+8%', signals:2},
];

const MOMENTUM_STYLE: Record<string,{bg:string,c:string}> = {
  HOT:   {bg:'rgba(46,204,113,0.1)', c:'#2ECC71'},
  RISING:{bg:'rgba(255,215,0,0.1)', c:'#ffd700'},
  STABLE:{bg:'rgba(0,180,216,0.1)', c:'#3498DB'},
};
    ...
    <div style={{minHeight:'100vh',background:'var(--bg-page)',fontFamily:"'Inter','Helvetica Neue',sans-serif"}}>
      ...
      <div style={{background:'linear-gradient(135deg,#FFFFFF,#060f1a)',padding:'28px 24px',borderBottom:'1px solid rgba(0,255,200,0.06)',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{maxWidth:'1540px',margin:'0 auto',position:'relative'}}>
          <div style={{fontSize:'10px',fontWeight:800,color:'#2ECC71',letterSpacing:'0.2em',marginBottom:'6px',fontFamily:'var(--font-display)'}}>CORRIDOR INTELLIGENCE</div>
          <h1 style={{fontSize:'28px',fontWeight:900,color:'var(--text-primary)',marginBottom:'6px'}}>Bilateral FDI Corridor Monitor</h1>
          <p style={{fontSize:'13px',color:'var(--text-muted)'}}>12 active corridors · Live flow data · Sector mapping · Strategic drivers</p>
        </div>
      </div>
      ...
      <div style={{maxWidth:'1540px',margin:'0 auto',padding:'20px 24px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'12px',marginBottom:'16px'}}>
          {[
            [CORRIDORS.length,'Active Corridors','All bilateral routes','#2ECC71'],
            [CORRIDORS.filter(c=>c.momentum==='HOT').length,'HOT Corridors','Accelerating flows','#2ECC71'],
            ...
            <div key={String(l)} style={{padding:'16px',background:'white',border:'1px solid var(--border)',borderRadius:'12px',borderTop:'2px solid '+c}}>
              <div style={{fontSize:'9px',color:'var(--text-light)',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:'5px'}}>{l as string}</div>
              <div style={{fontSize:'26px',fontWeight:900,color:String(c),fontFamily:'var(--font-mono)'}}>{v}</div>
        ...
        {/* Filters */}
        <div style={{background:'white',border:'1px solid var(--border)',borderRadius:'12px',padding:'12px 16px',marginBottom:'14px',display:'flex',gap:'10px',flexWrap:'wrap',alignItems:'flex-end'}}>
          ...
          <span style={{marginLeft:'auto',fontSize:'10px',color:'var(--text-light)',fontFamily:'var(--font-mono)',paddingBottom:'1px'}}>{filtered.length} corridors</span>
        </div>
          ...
          {filtered.map(cor => {
            ...
            return (
              <div key={cor.from+cor.to}
                ...
                <div style={{padding:'16px 18px'}}>
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'12px'}}>
                    <div style={{display:'flex',alignItems:'center',gap:'6px',flex:1}}>
                      ...
                      <div>
                        <div style={{fontSize:'13px',fontWeight:700,color:'#1A2C3E'}}>{cor.from}</div>
                        <div style={{fontSize:'9px',color:'var(--text-muted)'}}>Origin</div>
                      </div>
                    </div>
                    <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:'3px',flexShrink:0}}>
                      <div style={{fontSize:'14px',fontWeight:900,color:cor.color,fontFamily:'var(--font-mono)'}}>{cor.flow}</div>
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:'6px',flex:1,justifyContent:'flex-end'}}>
                      <div style={{textAlign:'right'}}>
                        <div style={{fontSize:'13px',fontWeight:700,color:'#1A2C3E'}}>{cor.to}</div>
                  ...
                  {/* Driver */}
                  <div style={{fontSize:'12px',color:'var(--text-secondary)',lineHeight:1.6,marginBottom:'10px'}}>{cor.driver}</div>
                  {/* Sectors */}
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    <div style={{display:'flex',gap:'12px'}}>
                      <span style={{fontSize:'10px',color:'var(--text-muted)'}}>Growth: <b style={{color:'var(--accent-green)',fontFamily:'var(--font-mono)'}}>{cor.growth}</b></span>
                      <span style={{fontSize:'10px',color:'var(--text-muted)'}}>Signals: <b style={{color:'#9b59b6',fontFamily:'var(--font-mono)'}}>{cor.signals}</b></span>
                    </div>
                ...
                {/* Expanded */}
                {isSel && (
                  <div style={{borderTop:'1px solid rgba(0,255,200,0.06)',padding:'14px 18px',background:'var(--bg-subtle)'}}>
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px',marginBottom:'12px'}}>
                      {[
                        [`GOSA ${cor.from}`,cor.gosaFrom,cor.color],
                        [`GOSA ${cor.to}`,cor.gosaTo,cor.color],
                        ['Annual Flow',cor.flow,'#ffd700'],
                        ['YoY Growth',cor.growth,'#2ECC71'],
                      ].map(([l,v,c]) => (
                        <div key={String(l)} style={{padding:'10px',background:'var(--bg-subtle)',borderRadius:'8px'}}>
                          <div style={{fontSize:'9px',color:'var(--text-light)',marginBottom:'3px',textTransform:'uppercase',letterSpacing:'0.06em'}}>{l as string}</div>
                          <div style={{fontSize:'16px',fontWeight:900,color:String(c),fontFamily:'var(--font-mono)'}}>{v}</div>
                        </div>
                      ))}
                    </div>
