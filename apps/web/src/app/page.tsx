...
// Single shared background color — used on html, body, root div, AND passed to Globe3D
const BG = '#080f1c'
...
export default function HomePage() {
  ...
  useEffect(() => {
    // Match html/body to same bg so no flash
  ...
  return (
    <>
      {/* Force same bg on everything so there's never a seam */}
      <style>{`
      ...
      <div style={{
        ...
        {/* ── ONE subtle grid — spans entire page, no difference left/right ── */}
        <div style={{
        ...
        <div style={{
          position:'absolute',left:0,right:0,top:0,height:'22%',zIndex:4,pointerEvents:'none',
          background:`linear-gradient(180deg,${BG}EE 0%,${BG}44 55%,transparent 100%)`,
        }}/>

        {/* ── Bottom fade ── */}
        <div style={{
          position:'absolute',left:0,right:0,bottom:0,height:'16%',zIndex:4,pointerEvents:'none',
          background:`linear-gradient(0deg,${BG}DD 0%,transparent 100%)`,
        }}/>

        {/* ══ GLOBE — right side, transparent WebGL, no background of its own ══ */}
        <div style={{
          position:'absolute',
          right:'-3%', top:'5%', bottom:'-3%',
          width:'62%',
          zIndex:10,
        }}>
          {ready && <Globe3D/>}
        ...
        <nav style={{
          ...
          <Link href="/" style={{textDecoration:'none',flexShrink:0}}>
            <div style={{display:'flex',flexDirection:'column',lineHeight:1}}>
              ...
              <span style={{fontSize:8.5,fontWeight:700,color:'rgba(255,255,255,0.32)',
                letterSpacing:'0.28em',textTransform:'uppercase',marginTop:2}}>MONITOR</span>
            </div>
          </Link>

          {/* About Us · Features · Contact Us — exact center */}
          <div style={{position:'absolute',left:'50%',transform:'translateX(-50%)',display:'flex',gap:2}}>
            {[['About Us','/about'],['Features','/dashboard'],['Contact Us','/contact']].map(([l,h])=>(
              <Link key={l} href={h} style={{
                ...
                borderRadius:7,letterSpacing:'0.015em',transition:'color 0.18s',
              }}
          ...
          <div style={{marginLeft:'auto',display:'flex',gap:6,alignItems:'center'}}>
            <Link href="/auth/login" style={{padding:'7px 14px',fontSize:13,fontWeight:500,
              color:'rgba(255,255,255,0.38)',textDecoration:'none',borderRadius:7,transition:'color 0.15s'}}
              onMouseEnter={e=>{(e.currentTarget as any).style.color='rgba(255,255,255,0.88)'}}
              onMouseLeave={e=>{(e.currentTarget as any).style.color='rgba(255,255,255,0.38)'}}>
              Sign In
            </Link>
            <Link href="/register" style={{
              padding:'7px 17px',fontSize:13,fontWeight:600,
        ...
        <div style={{
          ...
          {/* LARGE ROTATING QUOTE — pure white */}
          <div style={{
            minHeight:'clamp(120px,22vw,250px)',
            display:'flex',alignItems:'flex-start',
            width:'100%',marginBottom:38,
          }}>
            <div style={{
              opacity:op,
