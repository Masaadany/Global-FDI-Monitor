...
const INITIAL_ALERTS: Alert[] = [
  ...
  {id:10,type:'SYSTEM',title:'Trial period: 4 days remaining',body:'Your 7-day access expires in 4 days. Upgrade to Professional for unlimited access including API, unlimited reports, and weekly Intelligence Brief.',read:false,time:'5d ago',priority:'MED',link:'/pricing'},
];
...
const TYPE_COLOR: Record<string,string> = {SIGNAL:'#ffd700',GOSA_CHANGE:'#2ECC71',WATCHLIST:'#3498DB',SYSTEM:'#ff4466'};
    ...
    <div style={{minHeight:'100vh',background:'var(--bg-page)',fontFamily:"'Inter','Helvetica Neue',sans-serif"}}>
      ...
      <div style={{background:'linear-gradient(135deg,#FFFFFF,#060f1a)',padding:'20px 24px',borderBottom:'1px solid rgba(0,255,200,0.06)',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{maxWidth:'900px',margin:'0 auto',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:'12px',position:'relative'}}>
          <div>
            <div style={{fontSize:'10px',fontWeight:800,color:'#2ECC71',letterSpacing:'0.2em',marginBottom:'4px',fontFamily:'var(--font-display)'}}>ALERTS CENTRE</div>
            <h1 style={{fontSize:'22px',fontWeight:900,color:'var(--text-primary)',display:'flex',alignItems:'center',gap:'10px'}}>
              Notifications
          ...
          {unread > 0 && (
            <button onClick={markAllRead} style={{display:'flex',alignItems:'center',gap:'7px',padding:'8px 18px',background:'rgba(46,204,113,0.08)',border:'1px solid rgba(0,255,200,0.2)',borderRadius:'9px',cursor:'pointer',fontSize:'12px',fontWeight:700,color:'var(--accent-green)',fontFamily:'var(--font-ui)'}}>
      ...
      <div style={{maxWidth:'900px',margin:'0 auto',padding:'20px 24px'}}>
        ...
        <div style={{display:'flex',gap:'8px',marginBottom:'14px',flexWrap:'wrap',alignItems:'center'}}>
          {[['ALL','All'],['SIGNAL','Signals'],['GOSA_CHANGE','GOSA Updates'],['WATCHLIST','Watchlist'],['SYSTEM','System']].map(([v,l])=>(
            <button key={v} onClick={()=>setFilter(v)}
              style={{padding:'6px 14px',borderRadius:'20px',border:`1px solid ${filter===v?'rgba(46,204,113,0.5)':'rgba(255,255,255,0.07)'}`,background:filter===v?'rgba(46,204,113,0.08)':'rgba(255,255,255,0.03)',cursor:'pointer',fontSize:'11px',fontWeight:filter===v?700:400,color:filter===v?'#2ECC71':'rgba(232,244,248,0.45)',fontFamily:'var(--font-ui)',transition:'all 150ms'}}>
              {l}
            </button>
          ))}
          ...
          <span style={{marginLeft:'auto',fontSize:'10px',color:'var(--text-light)',fontFamily:'var(--font-mono)'}}>{filtered.length} alerts</span>
        </div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'7px'}}>
                ...
                <div style={{display:'flex',gap:'12px',alignItems:'flex-start'}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:'flex',gap:'7px',alignItems:'center',marginBottom:'4px',flexWrap:'wrap'}}>
                      ...
                      {alert.country && <span style={{fontSize:'11px',color:'var(--text-secondary)',fontWeight:600}}>{alert.country}</span>}
                      <span style={{fontSize:'10px',color:'var(--text-light)',fontFamily:'var(--font-mono)',marginLeft:'auto'}}>{alert.time}</span>
                    </div>
                    ...
                    <div style={{fontSize:'11px',color:'var(--text-muted)',lineHeight:1.65}}>{alert.body}</div>
                    <div style={{display:'flex',gap:'8px',marginTop:'10px'}}>
                      ...
                      {!alert.read && (
                        <button onClick={()=>markRead(alert.id)} style={{display:'flex',alignItems:'center',gap:'4px',padding:'5px 12px',background:'var(--bg-subtle)',border:'1px solid var(--border)',borderRadius:'6px',cursor:'pointer',fontSize:'11px',fontWeight:600,color:'var(--text-muted)',fontFamily:'var(--font-ui)'}}>
          ...
          {filtered.length === 0 && (
            <div style={{textAlign:'center',padding:'48px',color:'var(--text-light)',fontSize:'13px'}}>
