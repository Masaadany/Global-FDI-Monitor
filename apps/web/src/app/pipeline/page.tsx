...
const STAGES: {id:Stage;label:string;color:string;icon:string}[] = [
  ...
  {id:'analysis',     label:'Analysis',      color:'var(--accent-blue)',icon:'📊'},
  {id:'due_diligence',label:'Due Diligence', color:'#ffd700',icon:'🔬'},
  {id:'negotiation',  label:'Negotiation',   color:'#e67e22',icon:'🤝'},
  {id:'committed',    label:'Committed',     color:'var(--accent-green)',icon:'✅'},
];
...
const PRIORITY_STYLE = {
  ...
  LOW:  {bg:'rgba(0,180,216,0.1)', color:'#3498DB',border:'rgba(0,180,216,0.25)'},
};
    ...
    <div style={{minHeight:'100vh',background:'var(--bg-page)',fontFamily:"'Inter','Helvetica Neue',sans-serif"}}>
      ...
      <div style={{background:'linear-gradient(135deg,#FFFFFF,#060f1a)',padding:'20px 24px',borderBottom:'1px solid rgba(0,255,200,0.06)',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{maxWidth:'1540px',margin:'0 auto',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:'12px',position:'relative'}}>
          <div>
            <div style={{fontSize:'10px',fontWeight:800,color:'#2ECC71',letterSpacing:'0.2em',marginBottom:'4px',fontFamily:'var(--font-display)'}}>INVESTMENT PIPELINE</div>
            <h1 style={{fontSize:'20px',fontWeight:900,color:'var(--text-primary)'}}>Deal Pipeline Tracker</h1>
            <p style={{fontSize:'12px',color:'var(--text-muted)'}}>{items.length} deals · {items.filter(i=>i.priority==='HIGH').length} high priority · {items.filter(i=>i.stage==='committed').length} committed</p>
          </div>
          <div style={{display:'flex',gap:'8px',alignItems:'center'}}>
            <div style={{display:'flex',background:'var(--bg-subtle)',borderRadius:'8px',padding:'3px',border:'1px solid var(--border)'}}>
              {[['kanban','⬜ Board'],['table','☰ List']].map(([v,l]) => (
                <button key={v} onClick={()=>setView(v as any)}
                  style={{padding:'6px 14px',borderRadius:'6px',border:'none',cursor:'pointer',fontSize:'11px',fontWeight:view===v?700:400,fontFamily:'var(--font-ui)',background:view===v?'rgba(46,204,113,0.1)':'transparent',color:view===v?'#2ECC71':'rgba(232,244,248,0.45)',transition:'all 150ms'}}>
                  {l}
                </button>
              ))}
            </div>
            <button onClick={()=>setShowAdd(!showAdd)}
              style={{display:'flex',alignItems:'center',gap:'7px',padding:'9px 18px',background:'linear-gradient(135deg,#00ffc8,#00c49a)',color:'var(--primary)',border:'none',borderRadius:'9px',cursor:'pointer',fontSize:'12px',fontWeight:800,fontFamily:'var(--font-ui)',boxShadow:'0 4px 14px rgba(0,255,200,0.25)'}}>
      ...
      <div style={{maxWidth:'1540px',margin:'0 auto',padding:'20px 24px'}}>
        {/* Add Form */}
        {showAdd && (
          <div style={{background:'white',border:'1px solid rgba(0,255,200,0.18)',borderRadius:'12px',padding:'20px',marginBottom:'16px',boxShadow:'0 8px 32px rgba(0,0,0,0.4)'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr 1fr',gap:'12px',marginBottom:'12px'}}>
              {[
                {label:'Country',    key:'country',  opts:COUNTRIES.map(c=>({value:c,label:c})),     accent:'#2ECC71'},
                {label:'Sector',     key:'sector',   opts:SECTORS.map(s=>({value:s,label:s})),       accent:'#3498DB'},
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr auto auto',gap:'10px',alignItems:'flex-end'}}>
              <div>
                <label style={{fontSize:'9px',fontWeight:700,color:'#27ae60',display:'block',marginBottom:'4px',textTransform:'uppercase',letterSpacing:'0.08em'}}>Notes</label>
                <input value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))}
                  placeholder="Key notes, zone preferences, milestones..."
                  style={{width:'100%',padding:'8px 12px',background:'var(--bg-subtle)',border:'1px solid var(--border)',borderRadius:'8px',fontSize:'12px',fontFamily:'var(--font-ui)',outline:'none',color:'var(--text-primary)'}}/>
              </div>
              <button onClick={addItem} style={{padding:'9px 22px',background:'linear-gradient(135deg,#00ffc8,#00c49a)',color:'var(--primary)',border:'none',borderRadius:'9px',cursor:'pointer',fontSize:'12px',fontWeight:800,fontFamily:'var(--font-ui)',whiteSpace:'nowrap'}}>Add Deal</button>
              <button onClick={()=>setShowAdd(false)} style={{padding:'9px 16px',background:'var(--bg-subtle)',border:'1px solid var(--border)',borderRadius:'9px',cursor:'pointer',fontSize:'12px',fontWeight:600,color:'var(--text-muted)',fontFamily:'var(--font-ui)'}}>Cancel</button>
            </div>
          </div>
        )}
        ...
        {view === 'kanban' && (
          <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:'12px',alignItems:'start'}}>
            {STAGES.map(stage => {
              ...
              return (
                <div key={stage.id} style={{background:'white',borderRadius:'12px',border:'1px solid var(--border)',overflow:'hidden'}}>
                  <div style={{padding:'10px 14px',background:'var(--bg-subtle)',borderBottom:'1px solid rgba(255,255,255,0.04)',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    <div style={{display:'flex',alignItems:'center',gap:'7px'}}>
                      <span style={{fontSize:'14px'}}>{stage.icon}</span>
                      <span style={{fontSize:'11px',fontWeight:700,color:'var(--text-secondary)'}}>{stage.label}</span>
                    </div>
                    <span style={{fontSize:'11px',fontWeight:900,color:stage.color,fontFamily:'var(--font-mono)',padding:'2px 8px',background:`${stage.color}12`,borderRadius:'10px'}}>{stageItems.length}</span>
                  </div>
                  <div style={{padding:'8px',display:'flex',flexDirection:'column',gap:'7px',minHeight:'100px'}}>
                    {stageItems.map(item => {
                      const ps = PRIORITY_STYLE[item.priority];
                      return (
                        <div key={item.id} style={{background:'var(--bg-subtle)',borderRadius:'9px',padding:'10px 12px',border:'1px solid #ECF0F1',transition:'all 200ms ease'}}
                          ...
                          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'5px'}}>
                            <div style={{display:'flex',alignItems:'center',gap:'6px'}}>
                              ...
                              <div>
                                <div style={{fontSize:'11px',fontWeight:700,color:'var(--text-primary)'}}>{item.country}</div>
                                <div style={{fontSize:'9px',color:'var(--text-muted)'}}>{item.sector}</div>
                              </div>
                            </div>
                          ...
                          <div style={{fontSize:'11px',fontWeight:700,color:'var(--accent-green)',fontFamily:'var(--font-mono)',marginBottom:'4px'}}>{item.size}</div>
                          {item.notes && <div style={{fontSize:'10px',color:'var(--text-muted)',lineHeight:1.5,marginBottom:'6px'}}>{item.notes.slice(0,60)}{item.notes.length>60?'...':''}</div>}
                          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                            <span style={{fontSize:'9px',color:'var(--text-light)',fontFamily:'var(--font-mono)'}}>GOSA {item.gosa}</span>
                            <div style={{display:'flex',gap:'3px'}}>
                              {STAGES.findIndex(s=>s.id===item.stage) > 0 && (
                                <button onClick={()=>moveStage(item.id,-1)} style={{background:'none',border:'none',cursor:'pointer',color:'var(--text-light)',padding:'2px',fontSize:'10px',lineHeight:1}}>←</button>
                              )}
              ...
              <thead>
                <tr style={{background:'var(--bg-subtle)'}}>
                  {['Economy','Sector','Size','Stage','Priority','GOSA','Target','Notes','Actions'].map(h=>(
                    <th key={h} style={{padding:'10px 14px',textAlign:h==='Economy'||h==='Notes'?'left':'center',fontWeight:700,color:'var(--text-light)',textTransform:'uppercase',fontSize:'9px',letterSpacing:'0.08em',borderBottom:'1px solid rgba(0,255,200,0.06)',whiteSpace:'nowrap',fontFamily:"'JetBrains Mono',monospace'"}}>
                      {h}
                    </th>
                  ))}
                </tr>
                    ...
                    <tr key={item.id} style={{borderBottom:'1px solid rgba(255,255,255,0.025)',transition:'background 150ms'}}
                      ...
                      <td style={{padding:'11px 14px'}}>
                        <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
                          ...
                          <span style={{fontWeight:700,color:'var(--text-primary)'}}>{item.country}</span>
                        </div>
                      </td>
                      <td style={{padding:'11px 8px',textAlign:'center',fontSize:'11px',color:'var(--text-secondary)'}}>{item.sector}</td>
                      <td style={{padding:'11px 8px',textAlign:'center',fontSize:'11px',fontWeight:700,color:'var(--accent-green)',fontFamily:'var(--font-mono)'}}>{item.size}</td>
                      ...
                      <td style={{padding:'11px 8px',textAlign:'center',fontWeight:800,color:'var(--accent-green)',fontFamily:'var(--font-mono)'}}>{item.gosa}</td>
                      <td style={{padding:'11px 8px',textAlign:'center',fontSize:'11px',color:'var(--text-muted)'}}>{item.targetQ}</td>
                      <td style={{padding:'11px 14px',fontSize:'11px',color:'var(--text-muted)',maxWidth:'200px',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{item.notes||'—'}</td>
                      <td style={{padding:'11px 8px',textAlign:'center'}}>
                        <div style={{display:'flex',gap:'4px',justifyContent:'center'}}>
                          <Link href={'/country/'+(['SGP','MYS','THA','VNM','ARE','SAU','IND','IDN','KOR','JPN','GBR','DEU','USA','BRA','MAR','CAN','NLD','CHE','DNK','NZL'].find(id=>({Singapore:'SGP',Malaysia:'MYS',Thailand:'THA',Vietnam:'VNM',UAE:'ARE','Saudi Arabia':'SAU',India:'IND',Indonesia:'IDN','South Korea':'KOR',Japan:'JPN','United Kingdom':'GBR',Germany:'DEU','United States':'USA',Brazil:'BRA',Morocco:'MAR',Canada:'CAN',Netherlands:'NLD',Switzerland:'CHE',Denmark:'DNK','New Zealand':'NZL'} as any)[item.country]===id)||'MYS')} style={{padding:'4px 10px',background:'rgba(46,204,113,0.08)',border:'1px solid rgba(0,255,200,0.15)',borderRadius:'6px',textDecoration:'none',fontSize:'10px',fontWeight:700,color:'var(--accent-green)'}}>Profile</Link>
