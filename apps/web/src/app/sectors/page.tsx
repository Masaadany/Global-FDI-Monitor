...
const SECTORS = [
  {id:'ev-battery',    emoji:'🔋',name:'EV Battery',         momentum:'HOT',    color:'var(--accent-green)',gosa:88,topCountries:['🇻🇳 Vietnam','🇹🇭 Thailand','🇮🇩 Indonesia'],signals:3, fdi:'$25B',growth:'+42%',desc:'Fastest-growing FDI sector in Southeast Asia driven by EV supply chain diversification from China. Vietnam, Thailand, and Indonesia leading with combined $25B+ in committed investment for 2026.'},
  {id:'data-centers',  emoji:'🖥',name:'Data Centers',       momentum:'HOT',    color:'var(--accent-blue)',gosa:86,topCountries:['🇲🇾 Malaysia','🇸🇬 Singapore','🇦🇪 UAE'],signals:4,     fdi:'$18B',growth:'+38%',desc:'Hyperscaler expansion driving record greenfield investment. Malaysia 100% FDI liberalisation, UAE AI infrastructure commitment, Singapore land constraints creating Malaysia spillover opportunity.'},
...
const MOMENTUM_STYLE: Record<string, {bg:string,c:string,b:string}> = {
  HOT:    {bg:'rgba(46,204,113,0.1)',c:'#2ECC71',b:'rgba(46,204,113,0.3)'},
  RISING: {bg:'rgba(255,215,0,0.1)',c:'#ffd700',b:'rgba(255,215,0,0.25)'},
  STABLE: {bg:'rgba(0,180,216,0.1)',c:'#3498DB',b:'rgba(0,180,216,0.25)'},
    ...
    <div style={{minHeight:'100vh',background:'var(--bg-page)',fontFamily:"'Inter','Helvetica Neue',sans-serif"}}>
      ...
      <div style={{background:'linear-gradient(135deg,#FFFFFF,#060f1a)',padding:'28px 24px',borderBottom:'1px solid rgba(0,255,200,0.06)',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{maxWidth:'1540px',margin:'0 auto',position:'relative'}}>
          <div style={{fontSize:'10px',fontWeight:800,color:'#2ECC71',letterSpacing:'0.2em',marginBottom:'6px',fontFamily:'var(--font-display)'}}>SECTOR INTELLIGENCE</div>
          <h1 style={{fontSize:'28px',fontWeight:900,color:'var(--text-primary)',marginBottom:'6px'}}>Investment Sector Monitor</h1>
          <p style={{fontSize:'13px',color:'var(--text-muted)'}}>9 sectors · Momentum scoring · Top destinations · Live signals · GOSA-weighted</p>
        </div>
      </div>
      ...
      <div style={{maxWidth:'1540px',margin:'0 auto',padding:'24px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'12px',marginBottom:'20px'}}>
          {[
            [SECTORS.filter(s=>s.momentum==='HOT').length,'HOT Sectors','Immediate opportunity','#2ECC71'],
            ...
            [SECTORS.reduce((a,s)=>a+s.signals,0)+'','Active Signals','Across all sectors','#3498DB'],
          ].map(([v,l,s,c]) => (
            <div key={String(l)} style={{padding:'16px',background:'white',border:'1px solid var(--border)',borderRadius:'12px',borderTop:'2px solid '+c}}>
              <div style={{fontSize:'9px',color:'var(--text-light)',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:'6px'}}>{l as string}</div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'14px'}}>
          {SECTORS.map(sec => {
            ...
            return (
              <div key={sec.id} style={{background:'white',border:'1px solid var(--border)',borderRadius:'14px',overflow:'hidden',transition:'all 250ms ease',position:'relative'}}
                ...
                <div style={{padding:'18px 20px',borderBottom:'1px solid rgba(255,255,255,0.04)'}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'10px'}}>
                    <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                      ...
                      <div>
                        <div style={{fontSize:'16px',fontWeight:800,color:'var(--text-primary)'}}>{sec.name}</div>
                        <div style={{fontSize:'10px',color:'var(--text-muted)'}}>{sec.signals} active signals</div>
                      </div>
                    </div>
                  ...
                  <p style={{fontSize:'12px',color:'var(--text-muted)',lineHeight:1.7,marginBottom:'12px'}}>{sec.desc.slice(0,130)}...</p>
                  <div style={{display:'flex',gap:'14px',marginBottom:'10px'}}>
                    {[['FDI',sec.fdi,sec.color],['Growth',sec.growth,'#2ECC71'],['GOSA',sec.gosa,sec.color]].map(([l,v,c]) => (
                      <div key={String(l)}>
                        <div style={{fontSize:'9px',color:'var(--text-light)',marginBottom:'1px',textTransform:'uppercase',letterSpacing:'0.06em'}}>{l as string}</div>
                        <div style={{fontSize:'14px',fontWeight:800,color:String(c),fontFamily:'var(--font-mono)'}}>{v}</div>
                      </div>
                    ))}
                  </div>
                  <div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}>
                    {sec.topCountries.map(co => (
                      <span key={co} style={{fontSize:'10px',padding:'2px 8px',background:'var(--bg-subtle)',border:'1px solid var(--border)',borderRadius:'10px',color:'var(--text-secondary)',fontWeight:500}}>{co}</span>
                    ))}
                  </div>
                </div>
                <div style={{padding:'12px 20px',display:'flex',gap:'8px',background:'var(--bg-subtle)'}}>
                  ...
                  <Link href="/signals" style={{flex:1,padding:'7px',background:'var(--bg-subtle)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:'7px',textDecoration:'none',fontSize:'11px',fontWeight:600,color:'var(--text-muted)',textAlign:'center'}}>
                    Signals →
                  </Link>
                </div>
              </div>
